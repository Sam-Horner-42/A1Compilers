...
/* External objects */
extern digit syntaxErrorNumber; /* number of syntax errors reported by the parser */;
extern digit line; /* source code line number - defined in scanner.c */
...
extern empty startParser(empty);
extern digit startScanner(BufferPointer sc_buf);

static empty displayParser(BufferPointer ptrBuffer);
static bigdigit getParserFileSize(character* fname);
static empty callGarbageCollector(empty);
...
digit main4Parser(digit argc, word* argv) {
	...
	digit numParserErrors = 0;			/* Initializes the errors */
...
bigdigit getParserFileSize(character* fname) {
	FILE* input;
	bigdigit flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
empty displayParser(BufferPointer ptrBuffer) {
...
empty callGarbageCollector(empty) {
