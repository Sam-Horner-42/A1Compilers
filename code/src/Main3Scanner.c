 ...
 * -------------------------------------------------------------
 */
empty printScannerError(word fmt, ...);
empty displayScanner(BufferPointer ptrBuffer);
bigdigit getScannerFilesize(word fname);
...
digit main3Scanner(digit argc, word* argv) {
	...
	word fileName = argv[2];				/* Input file name */
	FILE* fptr;						/* Input file ptr */
